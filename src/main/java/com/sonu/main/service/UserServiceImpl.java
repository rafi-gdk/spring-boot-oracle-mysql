...
public class UserServiceImpl implements UserService {
    ...
    public UserResponse getUser(String userId) {
        ...
        try {
            Optional<UserDetails> user = repository.findByUserEmail(userId);
            if (user.isPresent()) {
    ...
    @Override
    public UserResponse userLogin(String userId, String password) {
        {
            UserResponse response;
            try {
                Optional<UserDetails> user = repository.findByUserEmail(userId);
                if (user.isPresent()) {
                    ...
                    if (null != userDetails.getUserPassword() && userDetails.getUserPassword().equals(password)) {
                        response = buildUserResponse(user.get());
                        log.info("getUser Response for " + userId + response);
                    } else {
                        throw new UserDataException("Password Incorrect");
                    }
                } else {
                    throw new UserDataException("UserNotFound");
                }
